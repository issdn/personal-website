<script lang="ts">
	...
	const importPainter = async () => {
		painter = (await import('$lib/components/pixel_painter/pixelPainterStore')).default;
		PixelPainterToolBox = (await import('$lib/components/pixel_painter/PixelPainterToolBox.svelte'))
			.default;
		PixelPainter = (await import('$lib/components/pixel_painter/PixelPainter.svelte')).default;
		painterReady = (await import('$lib/components/pixel_painter/pixelPainterStore')).painterReady;
	};
...
>
	...
	>
		...
		<div class="flex flex-col h-full gap-y-24">
			<section transition:fade={{ delay: 200 }} class="text-xl">
				<h1 class="text-5xl font-extrabold font-primary">{translation('title')}</h1>
		...
		>
			<span class="bg-primary dark:bg-light text-light dark:text-dark rounded-md px-2"
				>Made with Svelte</span
			>
			<a target="_blank" href="https://commons.wikimedia.org/wiki/File:Octicons-mark-github.svg"
				>Github Icon</a
			>
			<a target="_blank" href="https://icons8.com/icons/set/linkedin">Linkedin Icon</a>
	...
	>
		...
		{:else}
			...
			>
				<FormatPaint class="h-icon-sm"><desc>Turn on pixel art painter</desc></FormatPaint>
			</button>
		{/if}
	</div>
</div>
