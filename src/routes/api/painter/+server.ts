...
export async function GET() {
	const collection = client.db('karolbielski').collection(VERIFIED_PIXELS_DB_NAME);
	try {
		...
		await client.close();
		throw error(500, 'Something went wrong while fetching canvas from database.');
	} finally {
		await client.close();
	}
}
