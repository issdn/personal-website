...
const eraseCommand = (cellBoard: TCellBoard) => {
	...
	function execute(getRelativeCoordinates: GetRelativeCoordinatesFn) {
		...
		return (ctx: CanvasRenderingContext2D, painterContext: PainterContext) => {
			if (!lastCell) return;
			cellBoard.interpolate(lastCell.x, lastCell.y, cell.x, cell.y, (x: number, y: number) => {
...
const moveCommand = () => {
	...
	function start(getRelativeCoordinates: GetRelativeCoordinatesFn, x: number, y: number) {
	...
	function execute(getRelativeCoordinates: GetRelativeCoordinatesFn, x: number, y: number) {
		if (!isMoving) return;
...
const placeholderCommand = (cellBoard: TCellBoard) => {
	...
	function execute(getRelativeCoordinates: GetRelativeCoordinatesFn) {
		return (ctx: CanvasRenderingContext2D, painterContext: PainterContext) => {
			const cells = cellBoard.getCellBoardCells(...getRelativeCoordinates(), painterContext.size);
...
export { CommandInvoker, drawCommand, eraseCommand, moveCommand, placeholderCommand };
