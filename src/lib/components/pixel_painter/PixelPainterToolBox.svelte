<script lang="ts">
	...
	const handleColorKeydown = (e: KeyboardEvent, newColor: (typeof COLORS)[number][number]) => {
		if (e.key === ' ') {
	...
	const handleTouchMove = (e: TouchEvent) => {
		if (dragging) {
			position = calculateNewPosition(e.touches[0].clientX, e.touches[0].clientY);
		}
	};
	...
	const setPositionMiddle = () => {
		...
		position = { x: window.innerWidth / 2 - width / 2, y: 30 };
	};
	...
	onMount(() => {
		...
		return () => window.removeEventListener('resize', setPositionMiddle);
	});
</script>
...
>
	<div class="items-center flex flex-row justify-between">
		...
		>
			<DragIndicator class="fill-stone-500 h-8 w-8"><desc>Drag Toolbox</desc></DragIndicator>
		</span>
		...
		<div class="flex flex-row gap-x-2 justify-between pr-3">
			...
			<ToolButton
				...
				disabled={saving}
				on:click={handleSave}
				><Check class="h-8 w-8"><desc>Submit your painting</desc></Check></ToolButton
			>
		</div>
		...
		>
			...
			<ToolButton
				...
				><DragPan class="h-8 w-8"><desc>Move canvas</desc></DragPan></ToolButton
			>
			<span
		...
		<div transition:slide class="h-[180px] overflow-y-hidden mt-4">
			<div class="flex flex-col justify-between h-[180px]">
				...
				<div class="h-full px-2 flex flex-col justify-end gap-y-1">
					<label for="Brush Size" class="text-light dark:text-dark">Brush/Eraser Size</label>
					<RangeInput aria-label="Brush Size" name="Brush Size" bind:stepIndex={brushSize} />
				</div>
			</div>
		</div>
	{/if}
