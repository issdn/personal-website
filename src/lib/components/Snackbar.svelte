<script lang="ts">
	...
	const widthAnimate = (node: HTMLElement, { duration }: { duration: number }) => {
		return {
			duration,
			css: (t: number) => {
...
{#if show}
	...
	>
		...
		<button class="hover:opacity-80 text-5xl" on:click={handleClose}
			><Close class="h-8 fill-light/80" /></button
		>
		<div
